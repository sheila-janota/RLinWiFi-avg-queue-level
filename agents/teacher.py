...
class Logger:
            ...
            if experiment is None:
                try:
                    ...
                    kwargs = {
                        ...
                        "project_name": "rl-in-wifi-with-queue-level",
                        "workspace": "ABCD"
                    }
    ...
    def log_round(self, states, reward, cumulative_reward, info, loss, observations, step):
        ...
        if self.send_logs:
            ...
            #self.experiment.log_metric("Teste sheila", fairness, step=step)
        ...
        for i in range(EPISODE_COUNT):
            ...
            with tqdm.trange(steps_per_ep) as t:
                for step in t:
                    ...
                    #print("This is add_noise", add_noise)
...
class EnvWrapper:
    ...
    def _craft_commands(self, params):
        ...
        command = f'{waf_pwd} --run "RLinWiFi-master-original-avg-queue-level'
